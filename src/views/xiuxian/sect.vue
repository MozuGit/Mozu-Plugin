<template>
  <div class="sect-container">
    <a-card :bordered="false" class="fade-in-card">
      ...
      <a-table
        :columns="columns"
        :data-source="sectList"
        :loading="loading && sectList.length === 0"
    ...
    <a-modal
      ...
      :destroyOnClose="true"
    >
      <a-form ref="formRef" :model="formState" :rules="formRules" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
        ...
        <a-form-item label="宗门等级" name="level">
          <a-input-number
            ...
            :min="1"
            :max="maxLevel"
            :placeholder="`请输入宗门等级（最高${maxLevel}级）`"
            style="width: 100%"
          />
        </a-form-item>
...
import { TeamOutlined, EditOutlined, SyncOutlined } from '@ant-design/icons-vue'
...
const formRules = {
  name: [{ required: true, message: '请输入宗门名称', trigger: 'blur' }],
  level: [
