...
async function xiuxianText(msg, user_id, at, isMaster) {
  ...
  try {
    ...
    if (userData.event === 'user_init') {
      Text.push(['<@' + user_id + '>', '***', '**正在初始化你的修仙信息**', '***'].join('\n'))
    }
    xiuxian.recordDayActive(user_id)
    const id = userData.data.id
    const handler = commandHandlers[msg] || prefixHandlers.find((h) => h.prefix.test(msg))?.handler
    if (handler) {
...
const commandHandlers = {
  修炼: async (id, user_id, Text, msg, at, isMaster) => {
    ...
    switch (value.event) {
      ...
      case 'xiulian_cd':
        Text.push(
          ['<@' + user_id + '>', '***', '**当前正在CD中...**', '>剩余：' + value.data.outTime + '秒', '***'].join('\n')
        )
        break
      case 'in_retreat':
  ...
  修仙排行: async (id, user_id, Text) => {
    Text.push(['<@' + user_id + '>', '***', '**请选择你需要查看的排行榜**', '>排行榜仅展示前10名', '***'].join('\n'))
  ...
  我的称号: async (id, user_id, Text) => {
    ...
    if (userInfo.titles.length > 0) {
        ...
        titleList.push(
          [
            ...
            '有效期至：' +
              (title.validTime === 0
                ? '永久'
                : Math.floor(Date.now() / 1000) - title.validTime > 0
                  ? '已过期'
                  : formatTime(title.validTime)),
            setTitleBtn,
            '***',
          ].join('\n')
        )
      ...
      Text.push(
        ['<@' + user_id + '>', '***', '**称号列表**', '>称号可以通过排行榜等途径获取', '***', ...titleList].join('\n')
      )
    } else {
      Text.push(['<@' + user_id + '>', '***', '**你还没有称号呢**', '>称号可以通过排行榜等途径获取', '***'].join('\n'))
    }
    ...
    for (let beast of beasts) {
      beastText.push(
        [
          ...
          '名称：' + beast.name,
          '战力：' + beast.power,
          (await mqqapi.command('查看妖兽', '查看妖兽' + i, true)) +
            '  ' +
            (await mqqapi.command('猎杀妖兽', '猎杀妖兽' + i, true)),
          '***',
        ].join('\n')
      )
      i++
    }
    Text.push(['<@' + user_id + '>', '***', '**妖兽列表**', ...beastText].join('\n'))
  ...
  秘境列表: async (id, user_id, Text) => {
    ...
    const buildTexts = async (items, startIndex = 1) => {
      const promises = items.map(async (item, idx) => {
        ...
        return ['**秘境：' + item.name + '**', viewCmd + '  ' + exploreCmd].join('\n')
      })
    ...
    Text.push(
      [
        ...
        '**初级秘境**',
        '>门槛：' +
          (Config.xiuxian.Realm.Realms[Config.xiuxian.drop.secretRealm_limit.easy - 1]?.name ?? '无') +
          ((await xiuxian.getUserInfo(id)).realm.realm >= Config.xiuxian.drop.secretRealm_limit.easy
            ? ' **「已达成」**'
            : ''),
        ...easyText,
        '***',
        '**进阶秘境**',
        '>门槛：' +
          (Config.xiuxian.Realm.Realms[Config.xiuxian.drop.secretRealm_limit.medium - 1]?.name ?? '无') +
          ((await xiuxian.getUserInfo(id)).realm.realm >= Config.xiuxian.drop.secretRealm_limit.medium
            ? ' **「已达成」**'
            : ''),
        ...mediumText,
        '***',
        '**高级秘境**',
        '>门槛：' +
          (Config.xiuxian.Realm.Realms[Config.xiuxian.drop.secretRealm_limit.hard - 1]?.name ?? '无') +
          ((await xiuxian.getUserInfo(id)).realm.realm >= Config.xiuxian.drop.secretRealm_limit.hard
            ? ' **「已达成」**'
            : ''),
        ...hardText,
        '***',
  ...
  储物袋: async (id, user_id, Text) => {
    Text.push(['<@' + user_id + '>', '***', '**请选择你需要查看的背包**', '***'].join('\n'))
  ...
  我的宗门: async (id, user_id, Text) => {
    ...
    if (userInfo.sectInfo.id !== 0) {
      ...
      Text.push(
        ['<@' + user_id + '>', '***', '**你还没加入宗门呢**', '>点击' + (await mqqapi.command('加入宗门')), '***'].join(
          '\n'
        )
      )
    }
  ...
  创建宗门: async (id, user_id, Text) => {
    ...
    switch (value.event) {
      ...
      case 'create_sect':
        ...
        break
      case 'in_sect':
        Text.push(['<@' + user_id + '>', '***', '**你已经有宗门了**', '***'].join('\n'))
        break
    }
    Text.push(Button.sect)
  },
  ...
  宗门升级: async (id, user_id, Text) => {
    ...
    switch (value.event) {
      ...
      case 'sect_level_max':
        ...
        break
      case 'no_permission':
        Text.push(['<@' + user_id + '>', '***', '**你的宗门权限不足**', '>需长老及以上可操作', '***'].join('\n'))
        break
      case 'no_sect':
        Text.push(
          [
  ...
  宗门成员: async (id, user_id, Text) => {
    ...
    switch (value.event) {
      case 'members_list':
        ...
        Text.push(['<@' + user_id + '>', '***', '**宗门成员列表**', '***', memberList.join('\n'), '***'].join('\n'))
        break
      case 'no_sect':
        Text.push(
          [
  ...
  宗门签到: async (id, user_id, Text) => {
    ...
    switch (value.event) {
      case 'sign_in_success':
        ...
        break
      case 'is_signed':
        Text.push(['<@' + user_id + '>', '***', '**今天已经签到过了，明天再来吧**', '***'].join('\n'))
        break
      case 'no_sect':
        Text.push(
          [
    ...
    switch (value.event) {
      ...
      case 'audit_list':
        if (value.data.membersList.length === 0) {
          Text.push(['<@' + user_id + '>', '***', '**当前没有玩家申请加入宗门**', '***'].join('\n'))
        } else {
          let membersList = []
            ...
            membersList.push(
              [
                '>**ID：' +
                  member.id +
                  '     ' +
                  (await mqqapi.command('[同意]', '同意宗门成员' + member.id, true)) +
                  '     ' +
                  (await mqqapi.command('[拒绝]', '拒绝宗门成员' + member.id, true)) +
                  '**',
                '**修为：' + member.cult + '**',
                '**境界：' + member.realm + '**',
                '***',
          ...
          Text.push(['<@' + user_id + '>', '***', '**宗门待审核成员列表**', '***', membersList.join('\n')].join('\n'))
        }
        break
      case 'no_sect':
        Text.push(
...
const prefixHandlers = [
  {
    ...
    handler: async (id, user_id, Text, msg, at, isMaster) => {
      ...
      switch (value.event) {
        case 'realm_up_all':
          Text.push(['<@' + user_id + '>', '***', '**准备一键突破**', '>**突破成功率：未知**', '***'].join('\n'))
        ...
        case 'realm_up':
          Text.push(
            ['<@' + user_id + '>', '***', '**准备突破**', '>**突破成功率：' + value.data.rate + '%**', '***'].join('\n')
          )
  ...
  },
  {
    ...
    handler: async (id, user_id, Text, msg, at, isMaster) => {
      ...
      switch (value.event) {
        case 'pvp_end':
          Text.push(
            [
              ...
              '**切磋开始**',
              '>**你对ID: ' +
                (await mqqapi.command(value.data.random_id, '查询修仙者' + value.data.random_id)) +
                '发起切磋**',
              '***',
              '**切磋信息**',
        ...
        case 'pvp_cd':
          Text.push(
            ['<@' + user_id + '>', '***', '**当前正在CD中...**', '>**剩余' + value.data.pvp_cd + '秒**', '***'].join(
              '\n'
            )
          )
          break
        case 'cult_lack':
          Text.push(['<@' + user_id + '>', '***', '**你的修为不足5000**', '>暂时无法切磋', '***'].join('\n'))
          break
        case 'in_retreat':
          Text.push(await retreatText())
          break
  ...
  },
  {
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      if (userInfo) {
        ...
        Text.push(['<@' + user_id + '>', '***', '**未找到该玩家**', '>请确认该玩家是否存在或注册', '***'].join('\n'))
      }
      Text.push(Button.xiuxian)
    },
  },
  {
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      if (!beastInfo) {
        Text.push(['<@' + user_id + '>', '***', '**妖兽ID不存在**', '>请确认妖兽ID是否正确', '***'].join('\n'))
      } else {
        Text.push(
          [
            '<@' + user_id + '>',
            ...
            '名称：' + beastInfo.name,
            '战力：' + beastInfo.power,
            (await mqqapi.command('猎杀妖兽', '猎杀妖兽' + beast_id, true)) +
              '  ' +
              (await mqqapi.command('妖兽列表', '妖兽列表', true)),
            '***',
            '**奖励**',
  ...
  },
  {
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      if (!secretRealmInfo) {
        Text.push(['<@' + user_id + '>', '***', '**秘境ID不存在**', '>请确认秘境ID是否正确', '***'].join('\n'))
      } else {
        const pills = []
        const arts = []
        ...
        Text.push(
          [
            ...
            '门槛：' +
              (Config.xiuxian.Realm.Realms[Config.xiuxian.drop.secretRealm_limit[secretRealmInfo.level] - 1]?.name ??
                '无'),
      ...
      for (const pill of pills) {
        pillsText.push(
          [
            ...
            '使用：' + pill.cult + '修为',
            '出售：' + pill.sell_ls + '灵石',
            (await mqqapi.command('使用丹药', '使用丹药' + pill.id + ' 数量:1')) +
              '   ' +
              (await mqqapi.command('出售丹药', '出售丹药' + pill.id + ' 数量:1')),
            '***',
          ].join('\n')
        )
      }
      ...
      for (const art of arts) {
        artsText.push(
          [
            ...
            '功法加成：' + art.addition + ' %',
            '出售：' + art.sell_ls + '灵石',
            (await mqqapi.command('学习功法', '学习功法' + art.id, true)) +
              '   ' +
              (await mqqapi.command('出售功法', '出售功法' + art.id + ' 数量:1')),
            '***',
          ].join('\n')
        )
      }
      ...
      switch (value.event) {
        case 'use_pill_all':
          ...
          for (const pill of value.data.usePills) {
            usePillsText.push(
              ['>使用丹药：' + pill.name + ' * ' + pill.count, '获得修为：' + pill.cultAll, '***'].join('\n')
            )
            useCount += pill.count
          }
          Text.push(
        ...
        case 'lack_pill_count':
          ...
          break
        case 'no_pill':
          Text.push(['<@' + user_id + '>', '***', '**丹药不存在**', '>请确认丹药是否存在', '***'].join('\n'))
          break
        case 'in_retreat':
          Text.push(await retreatText())
          break
      ...
      switch (value.event) {
        case 'sell_pill_all':
          ...
          for (const pill of value.data.sellPills) {
            sellPillsText.push(
              ['>出售丹药：' + pill.name + ' * ' + pill.count, '获得灵石：' + pill.lsAll, '***'].join('\n')
            )
        ...
        case 'learn_art_all':
          ...
          Text.push(
            [
              ...
              ...learnArtsInsText,
              ...learnArtsText,
              ...(haslearnArtsText.length || learnArtsInsText.length || learnArtsText.length
                ? ['']
                : ['>什么功法也没有']),
              '***',
            ].join('\n')
        ...
        case 'learn_art':
          Text.push(
            [
              ...
              value.data.state
                ? '功法加成：' + value.data.artInfo.addition + ' %'
                : '修为：-' + value.data.artInfo.deduct_cult,
            ].join('\n')
          )
          break
        case 'learned_art':
          Text.push(['<@' + user_id + '>', '***', '**功法已学习**', '>这个功法你学习过了', '***'].join('\n'))
          break
        case 'no_art':
          Text.push(['<@' + user_id + '>', '***', '**功法不存在**', '>请确认功法是否存在', '***'].join('\n'))
          break
        case 'in_retreat':
          Text.push(await retreatText())
          break
      ...
      switch (value.event) {
        case 'sell_art_all':
          ...
          for (const art of value.data.sellArts) {
            sellArtsText.push(
              ['>出售功法：' + art.name + ' * ' + art.count, '获得灵石：' + art.lsAll, '***'].join('\n')
            )
            sellCount += art.count
          }
          Text.push(
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      switch (value.event) {
        ...
        case 'cancel_replace_sroot':
          Text.push(['<@' + user_id + '>', '***', '**取消替换灵根**', '>已取消替换灵根', '***'].join('\n'))
          break
        case 'invaild_sroot':
          Text.push(['<@' + user_id + '>', '***', '**灵根不存在**', '>灵根ID不存在', '***'].join('\n'))
          break
  ...
  },
  {
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      if (sectInfo.id !== 0) {
        ...
        Text.push(['<@' + user_id + '>', '***', '**未找到该宗门**', '>请确认该宗门是否存在或创建', '***'].join('\n'))
      }
      Text.push(Button.sect)
    },
  },
  {
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      switch (value.event) {
        case 'in_sect':
          Text.push(['<@' + user_id + '>', '***', '**你已经加入宗门了**', '***'].join('\n'))
          break
        case 'member_full':
          Text.push(
            [
        ...
        case 'join_sect_audit':
          Text.push(['<@' + user_id + '>', '***', '**已向宗门发起加入宗门申请**', '>请等待宗门审核', '***'].join('\n'))
          break
        case 'join_sect_success':
          Text.push(
            ['<@' + user_id + '>', '***', '**加入宗门成功**', '>宗门开启了无需宗门审核', '已直接加入宗门', '***'].join(
              '\n'
            )
          )
          break
        case 'not_sectid':
          Text.push(['<@' + user_id + '>', '***', '**找不到该宗门**', '>请确认该宗门是否存在或创建', '***'].join('\n'))
          break
      }
      Text.push(Button.sect)
    },
  },
  {
    ...
    handler: async (id, user_id, Text, msg, at) => {
      if (msg.includes('确认')) {
        ...
        Text.push(
          ['<@' + user_id + '>', '***', '**请再次确认退出宗门**', '>退出宗门后将失去宗门所有权益', '***'].join('\n')
        )
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      switch (value.event) {
        ...
        case 'not_transfer_id':
          Text.push(['<@' + user_id + '>', '***', '**找不到该玩家**', '>请确认该玩家是否在宗门内', '***'].join('\n'))
          break
        case 'no_permission':
          Text.push(['<@' + user_id + '>', '***', '**你的宗门权限不足**', '>需宗主才可操作', '***'].join('\n'))
          break
        case 'no_sect':
          Text.push(
            [
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      switch (value.event) {
        ...
        case 'member_has_sect':
          Text.push(['<@' + user_id + '>', '***', '**同意宗门成员失败**', '>对方已加入其他宗门', '***'].join('\n'))
          break
        case 'member_agreed':
          Text.push(['<@' + user_id + '>', '***', '**同意宗门成员成功**', '>玩家加入宗门成功', '***'].join('\n'))
          break
        case 'member_refused':
          Text.push(['<@' + user_id + '>', '***', '**拒绝宗门成员成功**', '>已拒绝玩家加入宗门', '***'].join('\n'))
          break
        case 'not_member':
          Text.push(
            ['<@' + user_id + '>', '***', '**待审核列表没有该玩家**', '>请确认玩家是否申请加入宗门或存在', '***'].join(
              '\n'
            )
          )
          break
    ...
    handler: async (id, user_id, Text, msg) => {
      ...
      switch (value.event) {
        case 'sect_set_audit_success':
          ...
          break
        case 'no_permission':
          Text.push(['<@' + user_id + '>', '***', '**你的宗门权限不足**', '>需副宗主及以上可操作', '***'].join('\n'))
          break
        case 'no_sect':
          Text.push(
            [
    ...
    handler: async (id, user_id, Text, msg) => {
      ...
      switch (value.event) {
        ...
        case 'not_allowed_newline':
          Text.push(['<@' + user_id + '>', '***', '**设置宗门名称失败**', '>宗门名称不允许换行', '***'].join('\n'))
          break
        case 'no_permission':
          Text.push(['<@' + user_id + '>', '***', '**你的宗门权限不足**', '>需宗门宗主可操作', '***'].join('\n'))
          break
        case 'no_sect':
          Text.push(
            [
    ...
    handler: async (id, user_id, Text, msg) => {
      ...
      switch (value.event) {
        ...
        case 'min_desc':
          ...
          break
        case 'not_allowed_newline':
          Text.push(['<@' + user_id + '>', '***', '**设置宗门简介失败**', '>宗门简介不允许换行', '***'].join('\n'))
          break
        case 'no_permission':
          Text.push(['<@' + user_id + '>', '***', '**你的宗门权限不足**', '>需宗门宗主可操作', '***'].join('\n'))
          break
    ...
    handler: async (id, user_id, Text, msg, at, isMaster) => {
      ...
      switch (value.event) {
        case 'sect_name_audit_all_success':
          Text.push(
            ['<@' + user_id + '>', '***', '**宗门名称审核全部' + match[1] + '**', '>操作成功！', '***'].join('\n')
          )
          break
        case 'sect_name_audit_success':
          ...
          break
        case 'not_sect_id':
          Text.push(['<@' + user_id + '>', '***', '**宗门不在待审核列表里**', '宗门ID：', '***'].join('\n'))
          break
        case 'sect_name_audit_list':
          ...
          for (const [ID, name] of Object.entries(value.data.sectAuditName)) {
            sectNameAuditListText.push(
              [
                '>宗门ID：' + ID,
                '待审核名称：' + name,
                (await mqqapi.command('[审核通过]', '宗门名称审核通过' + ID, true)) +
                  '      ' +
                  (await mqqapi.command('[审核驳回]', '宗门名称审核拒绝' + ID, true)),
                '***',
              ].join('\n')
            )
          }
          ...
          break
        case 'no_permission':
          Text.push(['<@' + user_id + '>', '***', '**权限不足**', '>请确认你是否有足够的权限', '***'].join('\n'))
          break
      }
    ...
    handler: async (id, user_id, Text, msg, at, isMaster) => {
      ...
      switch (value.event) {
        case 'sect_desc_audit_all_success':
          Text.push(
            ['<@' + user_id + '>', '***', '**宗门简介审核全部' + match[1] + '**', '>操作成功！', '***'].join('\n')
          )
          break
          ...
          for (const [ID, name] of Object.entries(value.data.sectAuditDesc)) {
            sectNameAuditListText.push(
              [
                '>宗门ID：' + ID,
                '待审核简介：' + name,
                (await mqqapi.command('[审核通过]', '宗门简介审核通过' + ID, true)) +
                  '      ' +
                  (await mqqapi.command('[审核驳回]', '宗门简介审核拒绝' + ID, true)),
                '***',
              ].join('\n')
            )
          }
    ...
    handler: async (id, user_id, Text, msg, at, isMaster) => {
      ...
      switch (value.event) {
        case 'sect_reset':
          ...
          break
        case 'no_sect':
          Text.push(['<@' + user_id + '>', '***', '**宗门不存在**', '>请确认该宗门是否存在', '***'].join('\n'))
          break
        case 'no_permission':
          Text.push(['<@' + user_id + '>', '***', '**权限不足**', '>请确认你是否有足够的权限', '***'].join('\n'))
          break
    ...
    handler: async (id, user_id, Text, msg) => {
      ...
      switch (value.event) {
        case 'set_sex_success':
          Text.push(['<@' + user_id + '>', '***', '**性别设置成功**', '>性别：' + value.data.sex, '***'].join('\n'))
          break
        case 'invalid_sex':
          Text.push(['<@' + user_id + '>', '***', '**性别设置失败**', '>请确认性别是否正确', '***'].join('\n'))
          break
        case 'in_is_sex':
          Text.push(['<@' + user_id + '>', '***', '**性别已设置**', '>无法再次设置性别', '***'].join('\n'))
          break
      }
      Text.push(Button.xiuxian)
    },
    ...
    handler: async (id, user_id, Text, msg) => {
      ...
      switch (value.event) {
        case 'set_title_success':
          Text.push(
            ['<@' + user_id + '>', '***', '**称号设置成功**', '>当前称号：' + value.data.title, '***'].join('\n')
          )
          break
        case 'invalid_title':
          Text.push(['<@' + user_id + '>', '***', '**称号设置失败**', '>请确认称号ID是否存在', '***'].join('\n'))
          break
      }
      Text.push(Button.title)
    },
  ...
  },
  {
    ...
    handler: async (id, user_id, Text, msg, at, isMaster) => {
      ...
      switch (value.event) {
        ...
        case 'invalid_cdks':
          Text.push(
            ['<@' + user_id + '>', '***', '**删除失败**', '>格式错误', '正确格式：删除兑换码xxx', '***'].join('\n')
          )
          break
      }
      Text.push(Button.cdk)
    ...
    handler: async (id, user_id, Text, msg, at) => {
      ...
      switch (value.event) {
        ...
        case 'cdk_used':
          ...
          break
        case 'invalid_cdk':
          Text.push(['<@' + user_id + '>', '***', '**兑换码不存在**', '>请确认兑换码是否存在', '***'].join('\n'))
          break
      }
      Text.push(Button.xiuxian)
    },
        ...
        case 'switch_success':
          Text.push(
            [
              ...
              '**切换ID成功**',
              '>**ID：' +
                (await mqqapi.command(id, '查询修仙者' + id)) +
                '  <==>  ' +
                (await mqqapi.command(switch_id, '查询修仙者' + switch_id)) +
                '**',
              '***',
            ].join('\n')
          )
...
async function buildUserInfo(userInfo) {
  return [
    ...
    '>**' +
      (userInfo.sectInfo.id !== 0
        ? await mqqapi.command('宗门：' + userInfo.sectInfo.name + '    ID：' + userInfo.sectInfo.id, '我的宗门', true)
        : await mqqapi.command('未加入宗门', '加入宗门')) +
      '**',
  ]
}
